...
use App\Support\Validation\ValidateDeliveryService;
use DateInterval;
...
class DeliveryService
{
    ...
    public function __construct(
        ...
        private ValidateDeliveryService $validateDeliveryService,
    ) {
    }
    ...
    public function generate(array $payload, array $currentUser): array
    {
        ...
        foreach ($data as $dateString => $routes) {
            ...
            foreach ($routes as $index => $routeData) {
                $validation = $this->validateDeliveryService->generateDeliveriesValidation(
                    $couriers, 
                    $vehicles,
                    $routeData,
                    $index,
    ...
    {
        $this->validateDeliveryService->persistDeliveryValidation($data);
